...
- Detects fiducials in any of Spot's cameras, shows it on the screen and shows distance to fiducial
- Asks user input for which fiducial to follow
...
```
python3 -m pip install -r requirements_fc.txt
```
...
```
python3 -m fiducial_follow ROBOT_IP --use-world-objects false
```
