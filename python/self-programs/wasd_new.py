...
class WasdInterface(object):
    ...
    def _power_on_robot(self):
        try:
            ...
            self.add_message("Power on command sent.")
            # Wait for power on
            for _ in range(20):
                state = self.robot_state
                if state and state.power_state.motor_power_state == robot_state_proto.PowerState.STATE_ON:
                    self.add_message("Robot is powered ON.")
                return True
            time.sleep(0.5)
            self.add_message("Timed out waiting for robot to power on.")
            return False
        except Exception as e:
            self.add_message(f"Error powering on robot: {e}")
        return False
    def start(self):
        ...
        # Power on robot automatically
        self._power_on_robot()
